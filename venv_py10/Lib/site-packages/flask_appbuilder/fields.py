...
from wtforms.fields import Field, SelectField, SelectFieldBase, StringField
...
class CaptchaField(Field):
    widget = StringField.widget
    def __init__(self, label='CAPTCHA', **kwargs):
        super().__init__(label=label, validators=[DataRequired(), Regexp(regex='^[a-zA-Z0-9]{7}$')], **kwargs)
        self.captcha_text = self.generate_captcha()

    def generate_captcha(self):
        letters = string.ascii_letters
        numbers = string.digits
        captcha_text = ''.join(random.choice(letters + numbers) for _ in range(7))

        # Generate CAPTCHA image using Pillow
        img = Image.new('RGB', (150, 100), color='white')
        draw = ImageDraw.Draw(img)
        draw.text((100, 50), captcha_text, fill='black')

        # Store CAPTCHA image in a temporary file
        static_folder = os.path.join(os.path.dirname(__file__), 'static')  # Get path to 'static' folder
        img_path = os.path.join(static_folder, 'appbuilder\\img\\captcha.png')
        img.save(img_path)
        return captcha_text, img_path

    def process_formdata(self, value):
        super().process_formdata(value)
        if value != self.captcha_text:
            self.errors = ['Invalid CAPTCHA']  # Set errors as a list
        else:
            self.data = value
